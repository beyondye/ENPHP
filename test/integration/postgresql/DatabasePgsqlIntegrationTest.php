...
class DatabasePgsqlIntegrationTest extends TestCase
{
    /**
     * 测试 PostgreSQL 数据库连接
     */
     ...
     * 测试 PostgreSQL 数据库操作
     */
    public function testPgsqlOperation()
    {
        try {
            // 使用 PostgreSQL 服务连接数据库
            $db = Database::instance('database.pgsql');
            ...
            // 如果数据库操作失败，标记测试为跳过
            $this->markTestSkipped('PostgreSQL operation failed: ' . $e->getMessage());
        }
    }
     ...
     * 测试 PostgreSQL 事务
     */
    public function testPgsqlTransaction()
    {
        try {
            // 使用 PostgreSQL 服务连接数据库
            $db = Database::instance('database.pgsql');
            ...
            // 如果数据库操作失败，标记测试为跳过
            $this->markTestSkipped('PostgreSQL transaction test failed: ' . $e->getMessage());
        }
    }
     ...
     * 测试 Database 实例缓存
     */
    public function testPgsqlInstanceCaching()
    {
        try {
            // 获取第一个实例
            $db1 = Database::instance('database.pgsql');
            $this->assertNotNull($db1);
            
            // 获取第二个实例（应该是缓存的）
            $db2 = Database::instance('database.pgsql');
     ...
     * 测试无效的 PostgreSQL 服务
     */
    public function testInvalidPgsqlService()
    {
        // 测试不存在的服务
        $this->expectException(DatabaseException::class);
        Database::instance('database.non_existent_pgsql');
    }
}